...
```{r, echo=FALSE}
selected_genes <- rownames(counts_VST) #28570
```
...
```{r}
pdf(file="Z://01_Stefano/Collaboration_Data/Figure_elements/Pf_correlation.pdf")
draw(image)
dev.off()
```
...
```{r}
pdf(file="Z://01_Stefano/Collaboration_Data/Figure_elements/Pf_globalstructure_R2E.pdf")
draw(ht2)
dev.off()
```
...
```
```{r}
rownames(sample_metadata) <- paste0("Pf_", rownames(sample_metadata) )
```
...
```{r}
pdf(file="Z://01_Stefano/Collaboration_Data/Figure_elements/Pf_globalstructure_PCA.pdf")
ggplot(PCAobject_top2000_ggplot,
       aes(PC1,PC2,fill=segment ))+
  geom_point(size=6, shape=21)+
  #scale_color_manual(values = rep("gray30", 3))+
...
```{r}
pdf(file="Z://01_Stefano/Collaboration_Data/Figure_elements/Pf_globalstructure_palette.pdf")
qplot(x=1:(length(gradient_palette)+0), y = 1, fill=factor(1:(length(gradient_palette)+0)), geom="tile") +
  scale_fill_manual(values = c(gradient_palette)) +
  theme_void()+
  theme(legend.position="none") 
...
```{r}
#counts_VST_selectedgenes <- counts_VST_selectedgenes[19:33]
#sample_metadata <- sample_metadata[19:33,]
#gradient_palette <- colorRampPalette(brewer.pal(9, "Reds")[3:9])(11)[7:11]

#counts_VST_selectedgenes <- counts_VST_selectedgenes[22:33]
#sample_metadata <- sample_metadata[22:33,]
#gradient_palette <- colorRampPalette(brewer.pal(9, "Reds")[3:9])(11)[8:11]

#counts_VST_selectedgenes <- counts_VST_selectedgenes[25:33]
#sample_metadata <- sample_metadata[25:33,]
#gradient_palette <- colorRampPalette(brewer.pal(9, "Reds")[3:9])(11)[9:11]

counts_VST_selectedgenes <- counts_VST_selectedgenes[c(16,17,18,seq(from=25, to=33))]
sample_metadata <- sample_metadata[c(16,17,18,seq(from=25, to=33)),]
gradient_palette <- colorRampPalette(brewer.pal(9, "Reds")[3:9])(11)[c(6,9,10,11)]
