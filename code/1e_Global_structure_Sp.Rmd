...
names(color_categorical_segment) <- unique(sample_metadata$segment)
...
```{r, echo=FALSE}
selected_genes <- rownames(counts_VST) #22270
```
...
```
```{r}
pdf(file="Z://01_Stefano/Collaboration_Data/Figure_elements/Sp_correlation.pdf")
draw(image)
dev.off()
```
...
```{r}
pdf(file="Z://01_Stefano/Collaboration_Data/Figure_elements/Sp_globalstructure_R2E.pdf")
draw(ht2)
dev.off()
```
...
load("Z://01_Stefano/Collaboration_Data/VST_Sp_corrected.rds")
counts_VST <- VST_Sp_corrected

load("Z://01_Stefano/Collaboration_Data/Annotations/Dictionaries_standard/Sp_samplemetadata_standard.rds")
sample_metadata <- SpGut_design3
rm(SpGut_design3)

rownames(sample_metadata) <- paste0("Sp_", rownames(sample_metadata))
```
...
```{r}
pdf(file="Z://01_Stefano/Collaboration_Data/Figure_elements/Sp_globalstructure_Pca.pdf")
ggplot(PCAobject_top2000_ggplot,
       aes(PC1,PC2,fill=segment ))+
  geom_point(size=6, shape=21)+
  #scale_color_manual(values = rep("gray30", 3))+
...
qplot(x=1:(length(gradient_palette)+0), y = 1, fill=factor(1:(length(gradient_palette)+0)), geom="tile") +
  scale_fill_manual(values = c(gradient_palette)) +
  theme_void()+
  theme(legend.position="none") 
```

```{r}
pdf(file="Z://01_Stefano/Collaboration_Data/Figure_elements/Sp_globalstructure_palette.pdf")
qplot(x=1:(length(gradient_palette)+0), y = 1, fill=factor(1:(length(gradient_palette)+0)), geom="tile") +
  scale_fill_manual(values = c(gradient_palette)) +
  theme_void()+
  theme(legend.position="none") 
...
```{r}
counts_VST_selectedgenes <- counts_VST_selectedgenes[9:36]
sample_metadata <- sample_metadata[9:36,]
gradient_palette <- colorRampPalette(brewer.pal(9, "Reds")[3:9])(9)[3:9]

#counts_VST_selectedgenes <- counts_VST_selectedgenes[13:36]
#sample_metadata <- sample_metadata[13:36,]
#gradient_palette <- colorRampPalette(brewer.pal(9, "Reds")[3:9])(9)[4:9]

#counts_VST_selectedgenes <- counts_VST_selectedgenes[17:36]
#sample_metadata <- sample_metadata[17:36,]
#gradient_palette <- colorRampPalette(brewer.pal(9, "Reds")[3:9])(9)[5:9]

#counts_VST_selectedgenes <- counts_VST_selectedgenes[9:24]
#sample_metadata <- sample_metadata[9:24,]
#gradient_palette <- colorRampPalette(brewer.pal(9, "Reds")[3:9])(9)[3:6]

#counts_VST_selectedgenes <- counts_VST_selectedgenes[25:36]
#sample_metadata <- sample_metadata[25:36,]
#gradient_palette <- colorRampPalette(brewer.pal(9, "Reds")[3:9])(9)[7:9]
```
