...
color_categorical_segment <- rep(c("dodgerblue3","darkorange2",  "indianred3", "lightskyblue","lightskyblue1", "sandybrown","lightgoldenrod1", "lightyellow", "lightcoral","lightpink"), each=1)
names(color_categorical_segment) <- unique(sample_metadata$segment)
ordered_levels_segment<- unique(sample_metadata$segment)
```

### gene selection (all) and replicate averaging

```{r, echo=FALSE}
selected_genes <- rownames(counts_VST) #37392
```
...
```
```{r}
pdf(file="Z://01_Stefano/Collaboration_Data/Figure_elements/Pd_correlation.pdf")
draw(image)
dev.off()
```
...
```
```{r}
pdf(file="Z://01_Stefano/Collaboration_Data/Figure_elements/Pd_globalstructure_R2E.pdf")
draw(ht2)
dev.off()
```
...
load("Z://01_Stefano/Collaboration_Data/Annotations/Dictionaries_standard/Pd_samplemetadata_standard.rds") #SpGut_design3
rownames(sample_metadata) <- paste0("Pd_", rownames(sample_metadata))
```
...
```{r}
pdf(file="Z://01_Stefano/Collaboration_Data/Figure_elements/Pd_globalstructure_PCA.pdf")
ggplot(PCAobject_top2000_ggplot,
       aes(PC1,PC2,fill=segment ))+
  geom_point(size=6, shape=21)+
  #scale_color_manual(values = rep("gray30", 3))+
...
```{r}
pdf(file="Z://01_Stefano/Collaboration_Data/Figure_elements/Pd_globalstructure_palette.pdf")
qplot(x=1:(length(gradient_palette)+0), y = 1, fill=factor(1:(length(gradient_palette)+0)), geom="tile") +
  scale_fill_manual(values = gradient_palette) +
  theme_void()+
  theme(legend.position="none") 
...
```{r}
load("Z://01_Stefano/Collaboration_Data/Annotations/Dictionaries_standard/Pd_samplemetadata_standard.rds") #sample_metadata
...
```{r}
counts_VST_selectedgenes <- counts_VST_selectedgenes[16:30]
sample_metadata <- sample_metadata[16:30,]
rownames(sample_metadata) <- paste0("Pd_", rownames(sample_metadata))
gradient_palette <- colorRampPalette(brewer.pal(9, "Reds")[1:9])(10)[6:10]

#counts_VST_selectedgenes <- counts_VST_selectedgenes[19:30]
#sample_metadata <- sample_metadata[19:30,]
#rownames(sample_metadata) <- paste0("Pd_", rownames(sample_metadata))
#gradient_palette <- colorRampPalette(brewer.pal(9, "Reds")[1:9])(10)[7:10]

#counts_VST_selectedgenes <- counts_VST_selectedgenes[22:30]
#sample_metadata <- sample_metadata[22:30,]
#rownames(sample_metadata) <- paste0("Pd_", rownames(sample_metadata))
#gradient_palette <- colorRampPalette(brewer.pal(9, "Reds")[1:9])(10)[8:10]
```
