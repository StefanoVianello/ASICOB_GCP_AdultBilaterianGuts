...
```{r, echo=FALSE}
selected_genes <- rownames(counts_VST) #29721
```
...
```{r}
pdf(file="Z://01_Stefano/Collaboration_Data/Figure_elements/Bf_globalstructure_R2E_allsegments.pdf")
draw(ht2)
dev.off()
```
...
###R2E seriation (subset)

```{r}
counts_VST_selectedgenes_all <- counts_VST_selectedgenes
counts_VST_selectedgenes <- counts_VST_selectedgenes[,seq(from=2, to =5)]
...
```{r}
pdf(file="Z://01_Stefano/Collaboration_Data/Figure_elements/Bf_globalstructure_R2E_subset.pdf")
draw(ht2)
dev.off()
```
...
gradient_palette <- colorRampPalette(brewer.pal(9, "Reds")[3:9])(length(unique(PCAobject_top2000_ggplot$segment))-1)
gradient_palette <- c(gradient_palette[1], gradient_palette)
...
ggplot(PCAobject_top2000_ggplot,
  ...
  geom_point(size=c(rep(5,4), rep(6,16)), shape=c(rep(25,4), rep(21,16)))+
  #scale_color_manual(values = rep("gray30", 3))+
  #scale_shape_manual(values = c(24, 23, 22))+ 
  #scale_color_manual(values = color_categorical_segment)+
...
```{r}
pdf(file="Z://01_Stefano/Collaboration_Data/Figure_elements/Bf_globalstructure_PCA.pdf")
ggplot(PCAobject_top2000_ggplot,
       aes(PC1,PC2,fill=segment ))+
  geom_point(size=c(rep(5,4), rep(6,16)), shape=c(rep(25,4), rep(21,16)))+
  #scale_color_manual(values = rep("gray30", 3))+
...
```{r}
pdf(file="Z://01_Stefano/Collaboration_Data/Figure_elements/Bf_globalstructure_palette.pdf")
qplot(x=1:(length(gradient_palette)+0), y = 1, fill=factor(1:(length(gradient_palette)+0)), geom="tile") +
  scale_fill_manual(values = c(gradient_palette)) +
  scale_color_manual(values = "black")+
  theme_void()+
...
load("Z://01_Stefano/Collaboration_Data/Annotations/Dictionaries_standard/Bf_samplemetadata_standard.rds") #BfAdultGut_design2
sample_metadata <- BfAdultGut_design2
#rownames(sample_metadata) <- paste0("Bf_", rownames(sample_metadata))
```
...
```{r}
counts_VST_selectedgenes <- counts_VST_selectedgenes[9:20]
sample_metadata <- sample_metadata[9:20,]
gradient_palette <- colorRampPalette(brewer.pal(9, "Reds")[3:9])(4)[2:4]
